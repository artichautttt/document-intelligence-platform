"""Tests de l'implémentation Qdrant de VectorStore (client en mémoire, sans serveur)."""
...
def store() -> QdrantStore:
    return QdrantStore(location=":memory:", collection_name=f"test-{uuid.uuid4()}")


class TestQdrantStore:
    def test_add_chunks_raises_on_empty_list(self, store: QdrantStore) -> None:
        with pytest.raises(EmptyChunkListError):
            store.add_chunks([])

    def test_query_returns_most_similar_chunk_first(self, store: QdrantStore) -> None:
        chunks = [
    ...
    def test_query_result_preserves_provenance(self, store: QdrantStore) -> None:
        chunk = _chunk(
    ...
    def test_add_chunks_is_idempotent_via_upsert(self, store: QdrantStore) -> None:
