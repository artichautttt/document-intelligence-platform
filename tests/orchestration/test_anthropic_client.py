...
class TestAnthropicLLMClient:
    def test_complete_returns_text_from_response(self) -> None:
        ...
        with patch.object(
            ...
            "create",
            return_value=_text_response("Reponse generee"),
        ) as mock_create:
    ...
    def test_complete_raises_llm_generation_error_on_api_error(self) -> None:
        client = AnthropicLLMClient(api_key="fake-key")
        api_error = anthropic.APIError(message="boom", request=MagicMock(), body=None)
        with (
            patch.object(client._client.messages, "create", side_effect=api_error),
    ...
    def test_complete_raises_when_response_has_no_text_block(self) -> None:
        ...
        with (
            patch.object(
                client._client.messages, "create", return_value=empty_response
            ),
            pytest.raises(LLMGenerationError),
        ):
            client.complete("prompt")
